...
class NetAccess(object):
    ...
    def __init__(self):
        ...
        log.info('Setting logged In status as False')
        self.loggedIn = False
    ...
    def refresh(self):
        if self.loggedIn:
            ...
            if r_refresh.status_code == 200:
                ...
                log.error('Refresh failed with %s at %s', r_refresh.status_code, self.refresh_url)
    ...
    def logout(self):
        if self.loggedIn:
            ...
            if r_logout.status_code == 200:
                ...
                log.error('Logout failed with %s at %s', r_logout.status_code, self.logout_url)
    ...
    def setup(self):
        try:
            ...
            if r_bing.status_code == 303:
                ...
                self.loggedIn = True
                log.info('KeepAlive from %s', p_refresh.search(r_auth.text).group('url'))
                log.info('Logout from %s', p_logout.search(r_auth.text).group('url'))
                return True
